...
let check_string_conv f s s_exp =
  Alcotest.(check string) "=" s_exp (f (C.of_string s))

let tc_argb =
  Alcotest.test_case "argb produces correct color" `Quick @@ fun () ->
  let t = C.argb (-0x01) 0x00 0xff 0x100 in
  Alcotest.(check int) "=" 0x00 (C.a t) ;
  Alcotest.(check int) "=" 0x00 (C.r t) ;
  Alcotest.(check int) "=" 0xff (C.g t) ;
  Alcotest.(check int) "=" 0xff (C.b t)

let tc_rgb =
  Alcotest.test_case "rgb produces correct color" `Quick @@ fun () ->
  let t = C.rgb (-0x01) 0x10 0x100 in
  Alcotest.(check int) "=" 0xff (C.a t) ;
  Alcotest.(check int) "=" 0x00 (C.r t) ;
  Alcotest.(check int) "=" 0x10 (C.g t) ;
  Alcotest.(check int) "=" 0xff (C.b t)
...
let tc_of_string_argb =
  ...
  Alcotest.check (module Color) "=" t_exp (C.of_string_argb "abcd1234") ;
  Alcotest.check (module Color) "=" t_exp (C.of_string_argb "#abcd1234") ;
...
let tc_of_string_rgb =
  ...
  Alcotest.check (module Color) "=" t_exp (C.of_string_rgb "cd1234") ;
  Alcotest.check (module Color) "=" t_exp (C.of_string_rgb "#cd1234") ;
...
let tc_of_string =
  ...
  let t_exp = C.argb 0xab 0xcd 0x12 0x34 in
  Alcotest.check (module Color) "=" t_exp (C.of_string "abcd1234") ;
  Alcotest.check (module Color) "=" t_exp (C.of_string "#abcd1234") ;
  let t_exp = C.argb 0xff 0xcd 0x12 0x34 in
  Alcotest.check (module Color) "=" t_exp (C.of_string "cd1234") ;
  Alcotest.check (module Color) "=" t_exp (C.of_string "#cd1234") ;
